...
import { pushToast, ToastRegion } from './toasts.js';
 ...
 * it, a record modal, and toasts, built with Chakra UI. `ChakraProvider`
 * mounts here rather than in `main.tsx` so the test adapter, which renders
 * `TicketsScreen` directly, gets the same styling context the real app does.
 * The toast region mounts before any of the loading, error, or ready states,
 * so its live region is in the DOM before the first toast.
 */
...
function ScreenBody({ fixture }: { fixture: FixtureOptions }) {
  ...
  const filters = useFilters(
    rows,
    () => setPage(1),
    () => pushToast('Filters cleared', 3000),
  );
