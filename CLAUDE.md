...
- **Import `@uilc/fixture/data` from application code.** Use `loadTickets` from `@uilc/fixture`. The static entry point folds the 240 rows into the bundle and inflates the size number. Tests may import it freely.
- **Re-export the generator from `@uilc/fixture`.** It reads and writes files, and pulling it into that entry point drags `node:fs` into every browser bundle.
...
- Run `pnpm --filter @uilc/<name> test` as you go. The 18 criteria are the definition of done, not your own reading of the spec.
...
- Update `comparison.json` honestly as you build. It carries the two counts a script cannot read off the code.
